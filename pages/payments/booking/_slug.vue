      ...
      <table class="table">
        <thead>
          <tr>
            ...
            <th class="has-text-right">{{$t('booking_requirement')}}</th>
        ...
        <tbody>
          <tr>
            ...
            <td class="has-text-right">{{offer.reservation}}%</td>
            <td class="has-text-right"><strong>{{numberWithDots(offer.reservation/100*offer.total)}} {{offer.currency}}</strong></td>
...
export default {
  ...
  async asyncData ({ app, params: { slug }}) {
      ...
      const payment = {
        order_id: `${slug}_booking_${(new Date()).getTime()}`,
        country: null,
  ...
  methods: {
    async getAmount(event) {
      if (this.offer.currency !== currencies[event.target.value]) {
        ...
        this.payment.amount = parseInt((this.offer.total*this.offer.reservation * exchange.rate)/100)
        this.local_currency_payment = `${this.numberWithDots(this.payment.amount)} ${currencies[event.target.value]}`
      } else {
        this.payment.amount = this.offer.total*this.offer.reservation/100
      }
    },
